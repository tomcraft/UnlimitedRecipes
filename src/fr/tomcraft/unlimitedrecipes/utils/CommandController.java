...
public class CommandController implements CommandExecutor
{
    ...
    /**
     * Registers all command handlers and subcommand handlers in a class,
     * matching them with their corresponding commands and subcommands
     * registered to the specified plugin.
     *
     * @param plugin
     *            The plugin whose commands will be considered for registration
     * @param handler
     *            An instance of the class whose methods will be considered for
     *            registration
     */
    public static void registerCommands(JavaPlugin plugin, Object handler)
    {
        for (Method method : handler.getClass().getMethods())
        {
            ...
            {
                if (CommandController.isCommandHandler(method))
                {
                    ...
                    if (plugin.getCommand(annotation.name()) != null)
                    {
                        ...
                        CommandController.handlers.put(plugin.getCommand(annotation.name()), handler);
                        CommandController.methods.put(plugin.getCommand(annotation.name()), method);
                    }
                }
                if (CommandController.isSubCommandHandler(method))
                {
                    ...
                    if (plugin.getCommand(annotation.parent()) != null)
                    {
                        ...
                        CommandController.subCommands.put(subCommand.toString(), subCommand);
                        CommandController.subHandlers.put(subCommand.toString(), handler);
                        CommandController.subMethods.put(subCommand.toString(), method);
                    }
                }
            }
        }
    ...
    /**
     * This is the method that "officially" processes commands, but in reality
     * it will always delegate responsibility to the handlers and methods
     * assigned to the command or subcommand
     * Beyond checking permissions, checking player/console sending, and
     * invoking handlers and methods, this method does not actually act on the
     * commands
     */
    @Override
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args)
    {
        ...
        {
            /*
             * Get the subcommand given and the handler and method attached to
             * it
             */
            SubCommand subCommand = new SubCommand(command, args[0]);
            subCommand = CommandController.subCommands.get(subCommand.toString());
            /*
             * If and only if the subcommand actually exists...
             */
            if (subCommand != null)
            {
                Object subHandler = CommandController.subHandlers.get(subCommand.toString());
                Method subMethod = CommandController.subMethods.get(subCommand.toString());
                /*
                ...
                if (subHandler != null && subMethod != null)
                {
                    ...
                    /*
                     * If the method requires a player and the subcommand wasn't
                     * sent by one, don't continue
                     */
                    ...
                    /*
                     * If the method requires a console and the subcommand
                     * wasn't sent by one, don't continue
                     */
                    ...
                    /*
                     * If a permission is attached to this subcommand and the
                     * sender doens't have it, don't continue
                     */
        ...
        /*
         * If a subcommand was successfully executed, the command will not reach
         * this point
         * Get the handler and method attached to this command
         */
        Object handler = CommandController.handlers.get(command);
        Method method = CommandController.methods.get(command);
        /*
         * If and only if both handler and method exist...
         */
        ...
        {
            /*
             * If the method requires a player and the command wasn't sent by
             * one, don't continue
             */
            ...
            /*
             * If the method requires a console and the command wasn't sent by
             * one, don't continue
             */
    ...
    /**
     * @author AmoebaMan
     *         An annotation interface that may be attached to a method to
     *         designate it as a command handler.
     *         When registering a handler with this class, only methods marked
     *         with this annotation will be considered for command registration.
     */
     ...
     * @author AmoebaMan
     *         An annotation interface that may be attached to a method to
     *         designate it as a subcommand handler.
     *         When registering a handler with this class, only methods marked
     *         with this annotation will be considered for subcommand
     *         registration.
     */
    @Retention(RetentionPolicy.RUNTIME)
